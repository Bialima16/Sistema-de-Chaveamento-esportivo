...
export const supabaseService={
  async loadAll(){
    ...
    const competitionsData=competitions.map(c=>{
      ...
      const compKnockout=Object.fromEntries(knockout.filter(k=>k.competition_id===c.id).map(k=>[k.id,{scoreA:k.score_a??'',scoreB:k.score_b??'',sets:k.sets||[],status:k.status,winnerId:k.winner_id,teamAOverride:k.team_a_id,teamBOverride:k.team_b_id}]));
  ...
  async updateMatch(id,values){assert(await supabase.from('matches').update({score_a:values.scoreA,score_b:values.scoreB,sets:values.sets||[],status:values.status,winner_id:values.winnerId||null}).eq('id',id))},
  async updateKnockout(competitionId,id,values){assert(await supabase.from('knockout_matches').update({score_a:values.scoreA,score_b:values.scoreB,sets:values.sets||[],status:values.status,winner_id:values.winnerId||null,team_a_id:values.teamA||null,team_b_id:values.teamB||null}).eq('competition_id',competitionId).eq('id',id))},
  ...
  async resetTournamentData(){
    const results=await Promise.all([
      ...
      supabase.from('knockout_matches').update({team_a_id:null,team_b_id:null,score_a:null,score_b:null,sets:[],winner_id:null,status:'Pendente'}).neq('id','__none__'),
